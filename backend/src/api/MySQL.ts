...
class MySQLApi
{
    ...
    {
        const result1 = await this.commandExecutor.ExecuteCommand("mysql -u root -e \"SELECT @@warning_count;\"");
        const result2 = await this.commandExecutor.ExecuteCommand("mysql -u root -e \"SELECT @@error_count;\"");
